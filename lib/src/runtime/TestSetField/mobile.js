...
var FormField = {
    ...
    asyncSetFieldProps: function (data, type, updateCascade) {
        ...
        var _this = this;
        var promise = asyncSetProps(_this, data, 'TestSet', 'material_settlement');
            ...
            React.createElement(Tabs, { tabs: tabs, initialPage: 0, renderTabBar: function (props) { return React.createElement(Tabs.DefaultTabBar, __assign({}, props, { page: 3 })); }, onChange: function (tab, index) {
                ...
                React.createElement("div", null,
                    React.createElement(List, null, this.state.checkData.map(function (item, index) {
                        ...
                        return (React.createElement(List.Item, { onClick: _this_1.methods().checkClick.bind(_this_1, item), key: index, multipleLine: true },
                            React.createElement("span", { style: style }, item.name +
                                "".concat(item.supplier ? '/' + item.supplier : '') +
                                "".concat(item.extend_four ? '/' + item.extend_four : ''))));
                    }))))));
